# Load Libraries
# install.packages(c("shiny", "openair", "dplyr", "gridExtra", "ggplot2"))
...
# Make sure that there is a data folder and it is within the Rshiny App folder with the code
...
# Helper function to aggregate data by time using openair's timeAverage
aggregateData <- function(data, aggOption) {
  if (aggOption == "Raw Data") {
    data
  } else if (aggOption == "Hourly Average") {
    timeAverage(data, avg.time = "hour")
  } else if (aggOption == "Daily Average") {
    timeAverage(data, avg.time = "day")
  }
}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("aggOption", "Aggregation",
                  choices = c("Raw Data", "Hourly Average", "Daily Average")),
...
server <- function(input, output, session) {
  ...
  output$plotOutput <- renderPlot({
    ...
    isolate({
      ...
      # Aggregate data as per the selected option
      data <- aggregateData(data, input$aggOption)
